...
def parse_json_file():
    with open(SUMMARY_RESULTS_DIR + "experiment_1_results_nonuniform.csv", "w") as f:
        f.write("graph,rounding_scheme,c,worst_case_guarantee\n")

    # Star graph
    for c_value in c:
        simple_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_star_20_order_0_tr_c{int(c_value*100)}_r100_sim10000.json", "r"))
        sim_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_star_20_order_0_mc_c{int(c_value*100)}_r100_sim10000.json", "r"))

        with open(SUMMARY_RESULTS_DIR + "experiment_1_results_nonuniform.csv", "a") as f:
    ...
    # Tree graph
    for c_value in c:
        simple_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_tree_3_4_order_0_tr_c{int(c_value*100)}_r100_sim10000.json", "r"))
        sim_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_tree_3_4_order_0_mc_c{int(c_value*100)}_r100_sim10000.json", "r"))
    ...
    # Complete graph
    for c_value in c:
        simple_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_complete_10_order_0_si_c{int(c_value*100)}_r100_sim10000.json", "r"))
        sim_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_complete_10_order_0_mc_c{int(c_value*100)}_r100_sim10000.json", "r"))
    ...
    # Bipartite graph
    for c_value in c:
        simple_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_bipartite_10_10_order_0_si_c{int(c_value*100)}_r100_sim10000.json", "r"))
        sim_scheme_data = json.load(open(f"{RAW_RESULTS_DIR}/nonuniform_bipartite_10_10_order_0_mc_c{int(c_value*100)}_r100_sim10000.json", "r"))
...
def plot_results():
    ...
    rounding_scheme_labels = {
        ...
        "simulated": "Simulated Rounding Scheme"
    }

    df = pd.read_csv(SUMMARY_RESULTS_DIR + "experiment_1_results_nonuniform.csv")
    ...
    plt.tight_layout()
    plt.savefig(f"{FIGURES_DIR}/experiment_1_results_nonuniform.png")
    plt.show()
